...
Now this command will first run the services as per their configurations inside `bolt.service.yaml` for each service. After that this will create a nginx configuration file `bolt.nginx.conf` and run the nginx server with the help of this configuration file.
